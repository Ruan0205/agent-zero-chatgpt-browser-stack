...
class ApiMessage(ApiHandler):
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        ...
        # API access must not silently turn a UI-visible chat into an expiring
        # temporary chat. Expiration is opt-in only when creating a new context.
        lifetime_hours = input.get("lifetime_hours")
        ...
        if context_id:
            ...
            # Older API versions set a default TTL on *every* continuation,
            # including normal UI chats. Remove only those legacy implicit TTLs.
            if (context.get_data("api_lifetime_explicit") is not True
                    and context.get_data("lifetime_hours") is not None):
                context.set_data("lifetime_hours", None)
        ...
        # Only a newly created API context with an explicit TTL is ephemeral.
        # Existing chats retain their normal persistence on API continuation.
        if not input.get("context_id") and lifetime_hours is not None:
            context.set_data("lifetime_hours", lifetime_hours)
            context.set_data("api_lifetime_explicit", True)
