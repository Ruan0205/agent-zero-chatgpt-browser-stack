...
# Agent Zero v2.12 centralizes safe, checksummed streaming in
# helpers.file_transfers. Register the extra formats here so downloads keep
# accurate Content-Type headers without replacing the new transfer safeguards.
MIME_OVERRIDES = {
    ".ts": "text/typescript",
    ".tsx": "text/tsx",
    ".jsx": "text/jsx",
...
for extension, content_type in MIME_OVERRIDES.items():
...
class DownloadFile(ApiHandler):
    ...
    async def process(self, input: Input, request: Request) -> Output:
        ...
        try:
            file_path = await runtime.call_development_function(resolve_download_path, file_path)
