...
def get_or_create_subordinate(
    ...
    *,
    ...
    if target_context_id and reset_requested:
        # Models occasionally combine a stale context id with an explicit reset.
        # Reset is unambiguous: create a fresh child and discard the stale id
        # instead of failing the whole parent run with a repairable tool error.
        target_context_id = ""
...
class Delegation(Tool):
    ...
    async def execute(self, message="", reset="", context_id="", **kwargs):
        ...
        try:
            result = await run_subordinate(
                self.agent, subordinate, message, attachments
            )
        except Exception as exc:
            # A child model/browser timeout must not strand the parent at
            # "Calling Subordinate Agent". Return a normal tool result so the
            # parent can inspect partial work and continue the assignment itself.
            result = (
                "[SUBORDINATE_FAILED_RECOVERABLY] "
                f"{type(exc).__name__}: {exc}. "
                "The subordinate did not provide a verified final answer. "
                "Continue the original task yourself, inspect any partial changes, "
                "run the required validations, and only then respond to the user."
            )
