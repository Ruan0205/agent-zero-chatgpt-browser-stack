...
class CleanupExpiredApiChats(Extension):
    ...
    async def execute(self, data: dict[str, Any] | None = None, **kwargs):
        ...
        for context in list(AgentContext.all()):
            # Legacy API calls assigned a default lifetime even to existing,
            # UI-visible chats. Only an explicit TTL on a newly created API
            # context may authorize removal; preserve all legacy histories.
