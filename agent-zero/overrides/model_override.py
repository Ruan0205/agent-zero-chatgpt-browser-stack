...
def browser_model_is_locked(ctx: AgentContext) -> bool:
    if ctx.get_data("browser_model_lock"):
        return True
    # Existing browser chats predate the explicit lock. Do not let a model
    # switch discard their already-established browser conversation either.
    name = str(model_config.get_chat_model_config(ctx.agent0).get("name") or "")
    return name.startswith("chatgpt-browser") and any(
        getattr(item, "type", None) == "user" for item in getattr(ctx.log, "logs", ())
    )
...
class ModelOverride(ApiHandler):
    async def process(self, input: dict, request: Request) -> dict | Response:
        ...
        if action == "get":
            ...
            if browser_model_is_locked(ctx):
                return Response(status=409, response="Browser-backed chat model is locked after the first message")
