...
def real_user_messages(agent, messages: list[str]) -> list[str]:
    """Drop Agent Zero's synthetic opening turn, never the user's real prompt."""
    if not messages:
        return messages
    try:
        placeholder = str(agent.read_prompt("fw.initial_user_message.md") or "").strip()
    except Exception:
        placeholder = "Hello!"
    if placeholder and messages[0].strip() == placeholder:
        return messages[1:]
    return messages
...
class RenameChat(Extension):
    async def execute(self, loop_data: LoopData = LoopData(), **kwargs):
        ...
        messages = real_user_messages(self.agent, messages)
        if not messages:
            return
