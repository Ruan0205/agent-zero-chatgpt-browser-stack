...
_INBOUND_DEDUP_SECONDS = 20.0


def _is_duplicate_or_agent_echo(msg: dict) -> bool:
    """Second-line protection if the bridge receives a self-chat echo."""
    body = re.sub(r"\s+", " ", str(msg.get("body", "") or "")).strip()
    if re.match(
        r"^(?:Queued message #\d+\.|Hello! 👋, I'm Agent Zero, your AI assistant\.)",
        body,
        re.IGNORECASE,
    ):
        return True
    media_type = str(msg.get("mediaType", "") or "")
    if not body and not media_type:
        return False
    sender = normalize_number(msg.get("senderNumber", "") or msg.get("senderId", ""))
    key = f"{sender}\0{media_type}\0{body}"
    now = time.monotonic()
    expired = [item for item, deadline in _recent_inbound.items() if deadline <= now]
    for item in expired:
        _recent_inbound.pop(item, None)
    if key in _recent_inbound:
        return True
    _recent_inbound[key] = now + _INBOUND_DEDUP_SECONDS
    return False
...
async def poll_messages(config: dict) -> None:
    ...
    # Security policy for self-chat deployments: fail closed.  An empty
    # allow-list must never mean "accept everybody", and dedicated/group
    # modes are intentionally unavailable in this installation.
    allowed_set = normalize_allowed_numbers(config.get("allowed_numbers"))
    if config.get("mode") != "self-chat" or not allowed_set:
        PrintStyle.error(
            "WhatsApp: refusing to poll because secure self-chat mode or "
            "the allowed-number list is missing"
        )
        return
    ...
    for msg in messages:
        try:
            ...
            if msg.get("isGroup", False) or sender_num not in allowed_set:
                PrintStyle.debug(
                    f"WhatsApp: securely ignored message from {sender_num} "
                    f"(senderId: {msg.get('senderId', '')}, allowed: {allowed_set})"
                )
                continue
            if _is_duplicate_or_agent_echo(msg):
                PrintStyle.debug("WhatsApp: ignored duplicate/self-generated self-chat message")
                continue
...
async def _dispatch_message(config: dict, msg: dict) -> None:
    ...
    existing = _find_chats_by_jid(chat_id, sender_number)
...
def _find_chats_by_jid(chat_id: str, sender_number: str = "") -> list[str]:
    """Return one logical self-chat even when WhatsApp alternates LID/phone JIDs."""
    results: list[tuple[bool, str]] = []
    normalized_sender = normalize_number(sender_number)
    for ctx_id, ctx in AgentContext._contexts.items():
        if not isinstance(ctx, AgentContext):
            continue
        same_jid = ctx.data.get(CTX_WA_CHAT_ID) == chat_id
        same_sender = bool(
            normalized_sender
            and normalize_number(ctx.data.get(CTX_WA_SENDER_NUMBER, "")) == normalized_sender
        )
        if not same_jid and not same_sender:
            continue
        sender_name = str(ctx.data.get(CTX_WA_SENDER_NAME, "") or "")
        # Prefer the established named conversation over a duplicate whose
        # display name is just the legacy phone number.
        results.append((not sender_name.isdigit(), ctx_id))

    results.sort(reverse=True)
    return [ctx_id for _, ctx_id in results]
...
async def send_wa_reply(
    ...
    allowed_set = normalize_allowed_numbers(config.get("allowed_numbers"))
    sender_num = normalize_number(context.data.get(CTX_WA_SENDER_NUMBER, ""))
    if (
        config.get("mode") != "self-chat"
        or not allowed_set
        or context.data.get(CTX_WA_IS_GROUP, False)
        or sender_num not in allowed_set
    ):
        return "WhatsApp reply blocked by secure self-chat allow-list"
    ...
    if response_text and mode == "self-chat":
    ...
    if attachments:
        ...
        for host_path in host_paths:
            try:
                # Meta AI results must be sent as image media so WhatsApp renders
                # an inline preview instead of a downloadable document card.
                is_meta_ai_image = os.path.basename(host_path).startswith("meta-ai-")
                result = await wa_client.send_media(
                    base_url,
                    chat_id,
                    host_path,
                    media_type="image" if is_meta_ai_image else "",
                    file_name="",
                )
                if result.get("error"):
...
async def _read_attachments_to_host(
    ...
    host_paths: list[str] = []
    for path in paths:
        # meta_ai_image stores the exact WhatsApp bytes in the shared Agent Zero
        # upload volume. Read them directly instead of asking the execution
        # runtime to resolve a bare filename in a different working directory.
        name = os.path.basename(str(path))
        if name.startswith("meta-ai-") and name == str(path):
            direct_path = os.path.join("/a0/usr/uploads", name)
            if os.path.isfile(direct_path):
                host_paths.append(direct_path)
                continue
